...
int Client_DSSE::saveState()
{
    ...
    Miscellaneous::write_array_to_file(
            FILENAME_KEYWORD_COUNTER_ARRAY, gcsDataStructureFilepath, this->keyword_counter_arr, MATRIX_ROW_SIZE);
    printf("OK!\n");
...
int Client_DSSE::sendFile(string filename, string path, int SENDING_TYPE)
{
    ...
    try
    {
        ...
        {
            n = (filesize - offset > SOCKET_BUFFER_SIZE) ? SOCKET_BUFFER_SIZE : (int)(filesize - offset);
...
int Client_DSSE::createEncrypted_data_structure()
{
    ...
    try
    {
        ...
        if(dsse->setupData_structure(this->T_W,
                                     this->T_F,
                                     this->keyword_counter_arr,
                                     this->block_counter_arr,
                                     this->block_state_mat,
...
int Client_DSSE::sendEncryptedIndex()
{
    ...
    do
    {
        cout << "       UPLOAD ENCRYPTED INDEX TO SERVER? (y/n)  \n **** ONLY SAY 'y' IF CLIENT AND SERVER ARE "
                "DEPLOYED IN TWO DIFFERENT MACHINES!! *** \n";
        cin >> choice;
...
int Client_DSSE::searchKeyword(string keyword, TYPE_COUNTER& res)
{
    ...
    try
    {
        ...
        dsse->searchToken(tau, keyword, this->T_W, this->keyword_counter_arr, this->masterKey);
        end = time_now;
...
int Client_DSSE::addFile(string filename, string path)
{
    ...
    try
    {
        ...
        dsse->requestBlock_index(
                adding_filename_with_path, block_index, this->T_F, this->lstFree_column_idx, this->masterKey);
        end = time_now;
        cout << std::chrono::duration_cast<std::chrono::nanoseconds>(end - start).count() << " ns" << endl;
...
int Client_DSSE::delFile(string filename, string path)
{
    ...
    try
    {
        ...
        start = time_now;
        dsse->requestBlock_index(
                deleting_filename_with_path, block_index, this->T_F, this->lstFree_column_idx, this->masterKey);
