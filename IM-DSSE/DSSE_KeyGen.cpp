 ...
 */
int DSSE_KeyGen::genRow_key(unsigned char* pOutData, int out_len, unsigned char* pInData, int in_len, MasterKey* pKey)
{
 ...
 * @return	0 if successful
 */
int DSSE_KeyGen::genMaster_key(MasterKey* pKey, prng_state* prng)
{
    ...
    if((error = Kraw->generate_128_PRK(pPRK, BLOCK_CIPHER_SIZE, pXTS, BLOCK_CIPHER_SIZE, pSKM, BLOCK_CIPHER_SIZE)) !=
       CRYPT_OK)
    {
    ...
    if((error = Kraw->generate_krawczyk_128_KDF(
                pKey->key1, BLOCK_CIPHER_SIZE, (unsigned char*)"key1", 4, pPRK, BLOCK_CIPHER_SIZE)) != CRYPT_OK)
    {
    ...
    if((error = Kraw->generate_krawczyk_128_KDF(
                pKey->key2, BLOCK_CIPHER_SIZE, (unsigned char*)"key2", 4, pPRK, BLOCK_CIPHER_SIZE)) != CRYPT_OK)
    {
        printf("Error calling krawczyk_128_kdf function: %d\n", error);
        exit(0);
    }
    ...
    if((error = Kraw->generate_krawczyk_128_KDF(
                pKey->key3, BLOCK_CIPHER_SIZE, (unsigned char*)"key3", 4, pPRK, BLOCK_CIPHER_SIZE)) != CRYPT_OK)
    {
 ...
 * @return	0 if successful
 */
int DSSE_KeyGen::enc_dec_preAESKey(MatrixType* output, MatrixType* input, unsigned char preKey[], TYPE_INDEX len)
{
    int ret = 0;
    TYPE_INDEX idx;
    idx = 0;
                                        ...
                                        MasterKey* pKey)
{
    ...
    else // key for search
    {
        ...
        {
            memcpy(&uchar_counter[BLOCK_CIPHER_SIZE / 2],
                   &col_counter_arr[block_index],
                   sizeof(col_counter_arr[block_index]));
