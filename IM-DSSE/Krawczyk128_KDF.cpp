...
int Krawczyk128_KDF::generate_128_PRK(
        unsigned char *pPRK, int PRK_len, unsigned char *pXTS, int XTS_len, unsigned char *pSKM, int SKM_len)
{
    int error = 0;
...
int Krawczyk128_KDF::generate_krawczyk_128_KDF(
        unsigned char *pKM, int KM_len, unsigned char *pCTXinfo, int CTXinfo_len, unsigned char *pPRK, int PRK_len)
{
    ...
    {
        cout << "Generating Random & Uniform Key \"" << pCTXinfo << "\" using Krawczyk Key Derivation Function..."
             << endl;
