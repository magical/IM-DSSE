                    ...
                    int direction)
{
    ...
    /* encrypt it */
    if((err = ccm_memory(
                cipher, key, keylen, uskey, nonce, noncelen, header, headerlen, pt, ptlen, ct, tag, taglen, direction)) !=
       CRYPT_OK)
    {
...
int aes128_ctr_encdec(unsigned char *pt, unsigned char *ct, unsigned char *key, unsigned char *ctr, size_t numBlocks)
{
    ...
    if((err = ctr_start(find_cipher("aes"), ctr, key, BLOCK_CIPHER_SIZE, 0, CTR_COUNTER_BIG_ENDIAN, &symmetric_ctr)) !=
       CRYPT_OK)
    {
...
int omac_aesni_process(omac_state *omac, unsigned char *key, const unsigned char *in, unsigned long inlen)
{
    ...
    if((omac->buflen > (int)sizeof(omac->block)) || (omac->buflen < 0) || (omac->blklen > (int)sizeof(omac->block)) ||
...
{
    unsigned char err;
    asm volatile("rdrand %0 ; setc %1" : "=r"(*therand), "=qm"(err));
    return (int)err;
}
